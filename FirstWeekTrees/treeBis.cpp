...
public:
    ...
    void findAllMSA(Node* node, vector<Node*> &vectorLeaves, int sizeCluster);
private:
    vector<Node*> getGroupNodes(Node* node,int sizeCluster);
    ...
    void insertToCluster(Node* node, OpDirection_Enum direction, vector<Node*> &nodesToMSA);
...
*/
vector<Node*> Tree::getGroupNodes(Node* node,int sizeCluster){
    ...
    if (!node->isLeave() && 
        ...
        insertToCluster(node,leftRight_t,nodesToMSA);
        insertToCluster(node,rightRight_t,nodesToMSA);
        insertToCluster(node,leftLeft_t,nodesToMSA);
        ...
        nodesToMSA.push_back(getNode(node,rightLeft_t));
        nodesToMSA.push_back(getNode(node,leftRight_t));       
...
/*
Private method
The function will insert a node in the vector, but first it will check if the sequence already exist or not.
*/
void Tree::insertToCluster(Node* node, OpDirection_Enum direction, vector<Node*> &nodesToMSA){
    Node* nodeDirection = new Node();
    nodeDirection = getNode(node,direction);
    int found=0;                      
    for(int i=0; i<nodesToMSA.size(); i++){
        if(nodesToMSA[i]->Sequence()==nodeDirection->Sequence()){
            found++;
        }
    }
    if (found==0){
        //cout<<"push_back: "<<nodeDirection->SequenceID()<<endl;
        nodesToMSA.push_back(nodeDirection); 
    }
...
/*
*/
void Tree::findAllMSA(Node* node, vector<Node*> &leavesVector, int sizeCluster){
//cout<<"**********************node-KEY "<<node->SequenceID()<<endl;
    vector <Node*> mSAVector;
    if (!leavesVector.empty()){             //we still have leaves
        mSAVector=getGroupNodes(node, sizeCluster);
        if(!mSAVector.empty()) {            //we have more deep
            ...
            //recursivity L/R
            findAllMSA(node->Left(),leavesVector,sizeCluster);
            findAllMSA(node->Right(),leavesVector,sizeCluster);
        }
...
/*void Tree::mountTree(Tree* tree, vector<string> seqList){
    int key=0;
    int maxLenght=0;
    for(int i=0; i<seqList.size(); i=i+4){
        Node* node1= new newNode(seqName[i],seqName[i+1]);
        Node* node2= new newNode(seqName[i+2],seqName[i+3]);
        if (seqList[i].size()<=seqList[i+2].size()){
            Node* parent = new newNode(seqList[i+1],seqList[i+1]);
            
            parent->setLeft(node1);
            parent->setRight(node2);
        }
        if(seqList[i+1].size()>maxLenght){      //first seq
            maxLenght=seqList[i+1].size();
            maxSeq= seqList[i];
        }else if(seqList[i+3].size()>maxLenght){      //second seq
            maxLenght=seqList[i+3].size();
            maxSeq= seqList[i];
        }
    }
}  */
...
//read the sequences from txt
    ...
    cout<< "text sequences"<<endl;
    for(int i=1; i<listSequence.size(); i=i+2){
        cout <<listSequence[i]<<endl;       //better to use map <seqName><seqString> ???
    }*/

    // Add nodes
    tree->addRoot(0,"Seq_07");

    tree->addNode(4,"Seq_03",tree->Root(),left_t);
    tree->addNode(12,"Seq_07",tree->Root(),right_t);

    tree->addNode(2,"Seq_02",tree->Root()->Left(),left_t);
    tree->addNode(6,"Seq_03",tree->Root()->Left(),right_t);
    tree->addNode(10,"Seq_05",tree->Root()->Right(),left_t);
    tree->addNode(14,"Seq_07",tree->Root()->Right(),right_t);

    tree->addNode(1,"Seq_01",tree->Root()->Left()->Left(),left_t);
    tree->addNode(3,"Seq_02",tree->Root()->Left()->Left(),right_t);
    tree->addNode(5,"Seq_03",tree->Root()->Left()->Right(),left_t);
    tree->addNode(7,"Seq_04",tree->Root()->Left()->Right(),right_t);
    tree->addNode(9,"Seq_05",tree->Root()->Right()->Left(),left_t);
    tree->addNode(11,"Seq_06",tree->Root()->Right()->Left(),right_t);
    tree->addNode(13,"Seq_07",tree->Root()->Right()->Right(),left_t);
    tree->addNode(15,"Seq_08",tree->Root()->Right()->Right(),right_t);
    ...
    tree->findAllMSA(tree->Root(),resultLeaves, sizeCluster);
